...
def consolidated_figure(h, pop_names, ele_pos, time_pts, fig, variable_dict):
    ...
    key_val = 'LFP'
    ax, im = get_specific(
        ax, h, pop_names, time_pts, ele_pos, variable_dict[key_val], key_val)
...
variable_dict = {
    'LFP': ['i'], 'AMPA+NMDA': ['i_AMPA', 'i_NMDA'], 'GABA A': ['i_gaba_a'],
    'Capacitive':
    ['i_cap'], 'Potassium': ['i_k'], 'Passive': ['i_pas'],
